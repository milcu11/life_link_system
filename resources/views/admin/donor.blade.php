...
<div class="bg-gray-50 min-h-screen py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <a href="{{ route('admin.donors') }}" class="text-sm text-gray-600 hover:text-gray-800" title="Back to donors"><i class="fas fa-arrow-left"></i></a>
                    ...
                    <div class="bg-gray-50 p-3 rounded">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                            ...
                            <div class="flex items-center justify-end">
                                <a href="{{ route('admin.donors.download', $donor) }}" class="px-3 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">Download</a>
                            </div>
                        </div>
                        ...
                        @if(!$donor->is_verified)
                            <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                                <div class="md:col-span-2">
                                    <form method="POST" action="{{ route('admin.donors.reject', $donor) }}" class="space-y-2">
                                        @csrf
                                        ...
                                        <textarea name="reason" rows="3" class="w-full mt-1 border rounded p-2 text-sm" placeholder="Explain why this registration is rejected"></textarea>
                                            <button type="submit" class="px-3 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition cursor-pointer">Reject</button>
                                    </form>
                                </div>
                                <div class="flex items-center justify-end gap-2">
                                    <form method="POST" action="{{ route('admin.donors.approve', $donor) }}">
                                        @csrf
                                        <button type="submit" class="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition cursor-pointer">Approve</button>
                                    </form>
                                </div>
                            </div>
                        @else
                            <div class="mt-4 text-sm text-gray-600">Already verified</div>
                        @endif
                    </div>
                @else
